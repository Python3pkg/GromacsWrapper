...
def filter_grompp_options(**kwargs):
    ...
    grompp_options = ('f','po','c','r','rb','n','p','pp','o','t','e',  # files
                      ...
                      'renum', 'norenum')
    grompp = dict((k,v) for k,v in list(kwargs.items()) if k in grompp_options)
    other =  dict((k,v) for k,v in list(kwargs.items()) if k not in grompp_options)
    return grompp, other
...
def edit_mdp(mdp, new_mdp=None, extend_parameters=None, **substitutions):
    ...
    **Example** ::
    ...
    substitutions = {k: v for k,v in list(substitutions.items()) if v is not None}
    ...
    with open(mdp) as src:
        logger.info("editing mdp = {0!r}: {1!r}".format(mdp, list(substitutions.keys())))
        for line in src:
...
class IndexBuilder(object):
    ...
    **Example**
    ...
    def names(self):
        ...
        return list(self.indexfiles.keys())
    ...
    def combine(self, name_all=None, out_ndx=None, operation='|', defaultgroups=False):
        ...
        ndxfiles.extend(list(self.indexfiles.values()))
    ...
    def __del__(self):
        try:
            for path in list(self.indexfiles.values()):
